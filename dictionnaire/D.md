...
Protocole par lequel deux participants au réseau Lightning peuvent ouvrir un canal en contribuant tous deux à la transaction d'ouverture du canal, et que l'on pourrait traduire par « financement partagé ». Cette transaction a donc alors au moins deux inputs (un pour chaque participant), et au moins un output (l'adresse multisignature correspondant au canal). Le protocole *dual funding* permet aux deux participants de se coordonner pour réaliser cette transaction collaborative à laquelle ils contribuent tous les deux des inputs. Il fait partie de la spécification du Lightning Network, ce qui permet en théorie de garantir que deux participants utilisant des implémentations différentes du protocole Lightning soient en mesure d'ouvrir un canal avec financement partagé.
